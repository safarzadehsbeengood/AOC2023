...
def get_piece_type(i, j):
    ...
    for direction in directions:
        ...
        if i + di < 0 or i + di >= len(grid):
            continue
        if j + dj < 0 or j + dj >= len(grid[i + di]):
            continue
    ...
    for piece_type in pipes:
        if len(reachable_directions) == len(pipes[piece_type]):
            if all([direction in pipes[piece_type] for direction in reachable_directions]):
                return piece_type
    return None
